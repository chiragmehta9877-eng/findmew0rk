...
export default function TwitterJobsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] dark:bg-[#0A192F] text-slate-900 dark:text-white font-sans relative">
      ...
      <ProtectedOverlay>
          ...
          <JobHero 
             title="X (Twitter) Community Jobs" 
             subtitle="Real-time job threads from Tech & Crypto communities."
             placeholder="Search 'Hiring', 'Freelance', or 'Contract'..."
             themeColor="#ffffff" 
          />
          ...
          <div className="container mx-auto px-4 pb-20 flex flex-col lg:flex-row gap-8">
            ...
            <main className="flex-1">
               ...
               <div ref={jobsHeadingRef} className="flex items-center justify-between mb-6 pt-2">
                  <h1 className="text-2xl font-bold flex items-center gap-2"><Twitter className="text-black dark:text-white" /> {selectedCategory} Threads</h1>
                  <p className="text-slate-500 dark:text-gray-400 text-sm">
                    {loading ? "Scanning..." : `Showing ${startIndex + 1}-${Math.min(startIndex + ITEMS_PER_PAGE, jobs.length)} of ${jobs.length} tweets`}
                  </p>
               </div>
                      ...
                      {currentJobs.map((job) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <p className="text-sm font-medium text-slate-600 dark:text-gray-300 mb-3">@{job.employer_name || "User"}</p>
                              <div className="mb-4"><span className="inline-flex items-center gap-1 bg-gray-100 dark:bg-white/5 px-2 py-1 rounded text-xs font-medium text-slate-500 dark:text-gray-400"><MapPin size={10} /> Remote / Global</span></div>
               ...
               {!loading && jobs.length === 0 && (
                 <div className="text-center py-20 bg-white dark:bg-[#112240] rounded-xl border border-dashed border-gray-300 dark:border-white/10">
                   ...
                   <p className="text-gray-500 text-sm">Try changing the category filter or check backend.</p>
                 </div>
               )}
            </main>
          </div>
              ...
              <>
                ...
                >
                   <div className="p-5 border-b border-gray-100 dark:border-white/10 flex justify-between items-center bg-gray-50 dark:bg-white/5">
                      <h3 className="font-bold text-lg">Filter Threads</h3>
                   ...
                   <div className="p-5 border-t border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5">
                      <button 
                        onClick={() => setIsFilterOpen(false)}
                        className="w-full py-3 bg-black dark:bg-white text-white dark:text-black rounded-xl font-bold shadow-lg shadow-gray-500/30 active:scale-95 transition-transform"
                      >
                        Show Results
                      </button>
                   </div>
